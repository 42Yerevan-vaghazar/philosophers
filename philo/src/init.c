...
/*   Updated: 2022/09/15 09:09:28 by vaghazar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_args(char **av, int ac, t_global *g)
{
	...
	g->mutex_fork = malloc(sizeof(pthread_mutex_t) * ft_atoi(av[1]));
	if ((!g->p->r || !g->mutex_fork)
		&& write(1, "Error: memory not allocated\n", 29))
		exit(1);
